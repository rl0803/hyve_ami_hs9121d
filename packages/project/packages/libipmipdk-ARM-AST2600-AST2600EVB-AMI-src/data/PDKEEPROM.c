...
#define EEPROM_24C256_PAGE_SIZE		0x40
...
EEPROM_Dev_Info_T m_SupEEPROM [] = {
	{ 0x10,  0,     EEPROM_24C256_PAGE_SIZE}, 
...
RWEEPROM (EEPROM_Info_T *pInfo, INT8U RWFlag)
{
    ...
    while (pInfo->Len > 0)
    {
        ...
        else
        {
            ...
            {
                 if(((int(*)(char *,u8,u8 *,u8 *,size_t,size_t))g_HALI2CHandle[HAL_I2C_RW]) (BusName,((u8)pInfo->SlaveAddr >> 1), WriteBuf, ReadBuf, NumWrite + writeoffset, NumRead) == -1)
                {
                     IPMI_WARNING ("Error accessing EEPROM\n");
                     return -1;
                }
