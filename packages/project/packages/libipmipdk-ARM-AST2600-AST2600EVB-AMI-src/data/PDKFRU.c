...
int PDK_RegisterAllFRUs(int BMCInst)
{
	...
	handle = hal_get_device_handle (0x68);
	/* Get all the device handles with the same apptype */
	if(handle != INVALID_HANDLE)
	{
		/* Get Device Map */
		if (-1 != hal_device_get_properties (handle,(u8*)&fruCfg))
		{
	...
	return  PDK_RegisterFRU (&FRUInfo);
#endif
return 0;
}
...
PDK_ReadFRUData (_NEAR_ INT8U* pReq, INT8U ReqLen, _NEAR_ INT8U* pRes, int BMCInst)
{
    ...
    if (retval != -1)
    {
        pFRUReadRes->CountReturned = (INT8U) retval;
    }
    else
    {
