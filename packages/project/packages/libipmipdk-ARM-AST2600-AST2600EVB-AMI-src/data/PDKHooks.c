...
PDK_PostInit (int BMCInst)
{
    ...
    PDK_RegisterSensorHooks(BMCInst);
...
{
	int ret = -1, i = 0;
	CoreFeatures_T corefeatures = { 0 };
	typedef struct {
		INT8U NetFn;
		INT8U Cmd;
	} reqCmd;

	RetrieveCoreFeatures(&corefeatures);
	...
	if(corefeatures.redfish_support && corefeatures.hhm_hostinterface_support)
	{
...
/*-----------------------------------------------------------------
* @fn PDK_ActivateFlashMode
* @brief Hook to Activate the BMC into flash mode
 * @param       Mode - Mode being passed from Yafu
 * @param	RetVal - Return Value for further handling
 * @param       BMCInst - BMC Instance
* @return   return <non-zero> - Successfully completed the OEM flash mode activate
*           return 0 - Perform default flash mode activate
*-----------------------------------------------------------------*/
//int PDK_ActivateFlashMode(INT16U Mode, INT16U BootOnly, int *RetVal, int BMCInst)
//{
//	UN_USED(BootOnly);
//	UN_USED(RetVal);
//	UN_USED(BMCInst);
