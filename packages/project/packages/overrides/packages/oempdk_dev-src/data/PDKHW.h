...
extern void OEMSELRecordOSShutdown(int BMCInst,INT8U type);
...
/*---------------------------------------------------------------------
 * @fn PDK_GetMiscChassisState
 *
 * @brief This control function is called to get the MiscChassisState.
 *
 * @param	*pMiscChassisState	- pointer to MiscChassisState which will have default value
 *                                unless its modified by oem.
 *----------------------------------------------------------------------*/
extern int PDK_GetMiscChassisState( INT8U *pMiscChassisState, int BMCInst);
/*---------------------------------------------------------------------------
 * @fn PDK_SwitchEMPMUX
 *
...
extern int PDK_SetThermalLimit(INT8U ExceptionAction,INT8U TempLimit,INT16U ExceptionTime,INT8U EntityInst,int BMCInst);
extern int PDK_ThermalMonitor(INT8U EnableMonitor,int BMCInst);
extern int PDK_GetTempReading(INT8U EntityInst,INT8U* CurrentTempReading,int BMCInst);
extern int PDK_Enable_SMI     (void);

/*WCS Interrupt handler*/
extern int PDK_ADRAssertHndlr (IPMI_INTInfo_T *pIntInfo, SensorInfo_T *pSensorInfo);
extern  int PDK_PSUAlertHndlr (IPMI_INTInfo_T *pIntInfo, SensorInfo_T *pSensorInfo);

#if defined(CONFIG_SPX_FEATURE_GLOBAL_ARCH_POWER8) || defined(CONFIG_SPX_FEATURE_GLOBAL_ARCH_POWER9)

#define SOFT_POWEROFF                   0x00
#define SOFT_REBOOT                     0x01
#define	FFS_PRIMARY_SIDE_FAIL			0x03
#define FFS_GOLDEN_SIDE_FAIL            0x04

#define CHASSIS_IDENTIFY_TIMEOUT	0xFF
#define CHASSIS_IDENTIFY_FORCE		0

extern int Platform_PowerOn(int BMCInst);
extern int Platform_PowerOff(int BMCInst);
extern void Platform_ClockCycle(int GNO,int num_clks);
extern void Platform_ApplyMagicPattern(int BMCInst);
extern int PDK_EnableCronusState();
extern void PDK_EnableBMCBoot();
extern int PDK_FSIEnable();
extern void OEMSELRecordOSShutdown(int BMCInst,INT8U type);
int PDK_SetOCCActiveSensor(INT8U SensorReading);
extern int PDK_PwrButtonHndlr(IPMI_INTInfo_T *pIntInfo, SensorInfo_T* pSensorInfo);
extern int PDK_ChassisIdentifyHndlr(IPMI_INTInfo_T *pIntInfo, SensorInfo_T* pSensorInfo);
