...
def Py_FileFromTemplate(indir,outfile,loopmacro,values,*extras):
	...
	for val in values:
		...
		for ext in range(0, int(len(extras) / 2)):
			emacro = extras[(ext * 2)]
...
def build_install():
	...
	Py_Delete(ImageTree+"/etc/pam_withunix")
	Py_Delete(ImageTree+"/etc/pam_wounix")
	...
	retval=Py_SoftLink("./"+PrjVars["MULTI_ARCH_LIB"]+"/security",ImageTree+"/lib/security")
	if retval != 0:
		return retval
...
#
	...
	if retval != 0:
		return retval
	retval=Py_AddToMacros("CONFIG_SPX_FEATURE_MMC_DUAL_IMAGE_NAME")
	if retval != 0:
		return retval
	retval=Py_AddToMacros("CONFIG_SPX_FEATURE_MMC_ROOT_SIZE")
	if retval != 0:
		return retval
	retval=Py_AddToMacros("CONFIG_SPX_FEATURE_EMMC_CONF_PARTITION_SIZE")
	if retval != 0:
		return retval		
	...
	if retval != 0:
		return retval
	retval=Py_AddToFeatures("CONFIG_SPX_FEATURE_MMC_BOOT=MMC_BOOT_SUPPORT")
	if retval != 0:
		return retval
	retval=Py_AddToFeatures("CONFIG_SPX_FEATURE_HW_EMMC_BOOT=HW_EMMC_BOOT_SUPPORT")
	if retval != 0:
		return retval
	retval=Py_AddToFeatures("CONFIG_SPX_FEATURE_HW_EMMC_BOOT_DUAL_IMAGE")
	if retval != 0:
		return retval
	retval=Py_AddToFeatures("CONFIG_SPX_FEATURE_FIRMWARE_TAR")
	if retval != 0:
		return retval
	retval=Py_AddToFeatures("CONFIG_SPX_FEATURE_MMC_INCLUDE_KERNEL")
	if retval != 0:
		return retval
	
	#  Get Number of Network Interfaces
	NicCount=1;
	if "CONFIG_SPX_FEATURE_GLOBAL_NIC_COUNT" in PrjVars:
	...
	retval=Py_FileFromTemplate("interfaces",ImageTree+"/etc/defconfig/interfaces","$IFNUM", list(range(0,NicCount)),"$TYPE",NicConfig)
	if retval != 0:
		return retval
	...
	retval=Py_FileFromTemplate("dns.conf",ImageTree+"/etc/defconfig/dns.conf","$IFNUM", list(range(0,NicCount)))
	if retval != 0:
		return retval
	retval =Py_CopyFile(ImageTree+"/etc/defconfig/dns.conf", ImageTree+"/conf/dns.conf")
	if retval != 0:
		return retval
	if os.path.exists(ImageTree+"/etc/logrotate.conf"):
			os.chmod(ImageTree+"/etc/logrotate.conf",0o644)
	if os.path.exists(ImageTree+"/conf/rotate.conf"):
			os.chmod(ImageTree+"/conf/rotate.conf",0o644)

	#Check for the crontab file and change permissions to 644, else cron jobs will not run
	if os.path.exists(ImageTree+"/etc/crontab"):
			os.chmod(ImageTree+"/etc/crontab", 0o644)
...
#   	  		return retval
	...
	if  "CONFIG_SPX_FEATURE_MDNS_SUPPORT" in PrjVars:
		...
		if retval != 0:
			return retval
		retval = Py_AppendFile("messagebus:x:106:108:Dbus daemon,,,:/var/run/dbus:/bin/false", ImageTree+"/etc/defconfig/passwd")
		if retval != 0:
			return retval

	if "CONFIG_SPX_FEATURE_GCOV_SUPPORT" in PrjVars:
		rc_file = ImageTree + "/etc/init.d/rc"
		command = "sed -i '/exit 0/iexport GCOV_PREFIX=/mnt/gcov' " + rc_file
		retval = Py_Execute(command)
		if retval != 0:
			return retval           
	retval = Py_AppendFile("syslog:x:404:541:rsyslog daemon,,,:/var/log:/bin/false", ImageTree+"/etc/defconfig/passwd")
	if retval != 0:
		return retval			
